...
/* ── Stat row helper (shared by multiple types) ──────────────────────────── */
function StatsSection({ profile, set, count = 3 }) {
  const pairs = [
    ["stat1Value", "stat1Label", "100+",  "Happy Clients"],
    ["stat2Value", "stat2Label", "5.0★",  "Average Rating"],
    ["stat3Value", "stat3Label", "10+",   "Years Experience"],
    ["stat4Value", "stat4Label", "200+",  "Projects Done"],
  ].slice(0, count);
  ...
  return (
    <Section title="📊 Stats Bar">
      ...
      <Grid container spacing={2}>
        {pairs.map(([nKey, lKey, nDef, lDef], i) => (
...
/* ── Services list (hairdresser: name + duration + price) ────────────────── */
function HairdresserServicesSection({ profile, set, brandColor }) {
      ...
      {services.map((svc, i) => (
        <Box key={i} sx={{ border: "1px solid #eee", borderRadius: 2, p: 2, mb: 1.5 }}>
          ...
          <Grid container spacing={1.5}>
            <Grid item xs={12} sm={6}><TextField fullWidth size="small" label="Name" value={svc.name || ""} onChange={e => update(i, "name", e.target.value)} /></Grid>
            <Grid item xs={6} sm={3}><TextField fullWidth size="small" label="Duration" placeholder="60 min" value={svc.duration || ""} onChange={e => update(i, "duration", e.target.value)} /></Grid>
            <Grid item xs={6} sm={3}><TextField fullWidth size="small" label="Price (£)" placeholder="65" value={svc.price || ""} onChange={e => update(i, "price", e.target.value)} /></Grid>
          </Grid>
        </Box>
      ))}
    ...
    <Section title={title}>
      {services.map((svc, i) => (
        <Box key={i} display="flex" gap={1} mb={1}>
          <TextField fullWidth size="small" label={`Item ${i + 1}`}
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label="Eyebrow / Tagline"
              placeholder="London's Premier Barbershop"
          ...
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label="Heading Line 1"
              placeholder="Precision Cuts,"
              value={profile.heroHeadingLine1 || ""}
              onChange={e => set("heroHeadingLine1", e.target.value)} />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField fullWidth size="small" label="Heading Line 2 (accent colour)"
              placeholder="Sharp Fades."
          ...
          <Grid item xs={12}>
            <TextField fullWidth size="small" label="Sub-text" multiline rows={2}
              placeholder="Expert barbers dedicated to the craft…"
...
function HairdresserContent({ profile, set, brandColor }) {
  return (
    <>
      ...
      <HairdresserServicesSection profile={profile} set={set} brandColor={brandColor} />
      <StatsSection profile={profile} set={set} count={3} />
    </>
  );
}
...
/* ══════════════════════════════════════════════════════════════════════════
   DECORATOR content sections
══════════════════════════════════════════════════════════════════════════ */
function DecoratorContent({ profile, set, brandColor }) {
  const portfolioItems = profile.portfolioItems?.length > 0
    ? profile.portfolioItems
    : [{ before: "", after: "", label: "" }];

  const updatePortfolio = (i, field, val) => {
    const updated = portfolioItems.map((p, idx) => idx === i ? { ...p, [field]: val } : p);
    ...
    <>
      ...
      <Section title="🛠️ Services Section">
        <Grid container spacing={2} mb={2}>
          ...
          <Grid item xs={12}>
            <ImageField label="Services section image"
              value={profile.servicesImage || ""}
              onChange={v => set("servicesImage", v)} />
          </Grid>
        </Grid>
      ...
      <StatsSection profile={profile} set={set} count={4} />
    </>
  );
}

/* ══════════════════════════════════════════════════════════════════════════
   PT / TRAINER content sections  (pre-existing, unchanged)
══════════════════════════════════════════════════════════════════════════ */
function TrainerContent({ profile, set, brandColor }) {
      ...
      <Section title="🦸 Hero Section" defaultExpanded>
        <Grid container spacing={2.5}>
          ...
          <Grid item xs={12}>
            ...
            <ImageField label="About / Hero Image"
...
/* ══════════════════════════════════════════════════════════════════════════
   ROOT EXPORT
══════════════════════════════════════════════════════════════════════════ */
export default function WebsiteTab({ profile, setProfile, brandColor, businessType }) {
  ...
  return (
    <Box>
      <Typography variant="h6" fontWeight={800} mb={0.5}>
        {typeLabel} Content Editor
      </Typography>
      <Typography variant="body2" color="text.secondary" mb={3}>
        Edit every piece of text and image on your public page. Press <strong>Save Profile</strong> in the header to publish changes.
      </Typography>

      {type === "barber"      && <BarberContent      profile={profile} set={set} brandColor={brandColor} />}
      {type === "hairdresser" && <HairdresserContent profile={profile} set={set} brandColor={brandColor} />}
      {type === "decorator"   && <DecoratorContent   profile={profile} set={set} brandColor={brandColor} />}
      {type === "trainer"     && <TrainerContent     profile={profile} set={set} brandColor={brandColor} />}
    </Box>
  );
}
