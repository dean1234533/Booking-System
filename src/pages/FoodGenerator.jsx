...
import {
  Box, Typography, CircularProgress, Chip, Grid, Card, CardContent,
  Tabs, Tab, TextField, MenuItem, Divider, Accordion, AccordionSummary,
  AccordionDetails, Alert, Slider,
} from "@mui/material";
...
export default function FoodGenerator() {
  ...
  const [status,     setStatus]     = useState("loading"); // loading | active | inactive | error
  ...
  const guideSections = [
    NUTRITION_GUIDE.preWorkout,
    NUTRITION_GUIDE.postWorkout,
    NUTRITION_GUIDE.breakfast,
    NUTRITION_GUIDE.lateNight,
    NUTRITION_GUIDE.hydration,
    NUTRITION_GUIDE.shakesForBusy,
  ];

  // ── Loading / inactive screens ────────────────────────────────────────────
  if (status === "loading") {
    return (
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "#0a0a0a", color: "#fff", fontFamily: "'Inter', sans-serif" }}>
      ...
      <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, md: 4 }, py: 4 }}>
        ...
        {/* ── Goal Selection ── */}
        <Box sx={{ mb: 5 }}>
          <Typography variant="h5" fontWeight={900} sx={{ mb: 1 }}>Choose Your Goal</Typography>
          ...
          <Card sx={{ bgcolor: "#111", border: "1px solid #222", borderRadius: 3, p: 3 }}>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6} md={2}>
                <TextField
                  select fullWidth label="Gender" size="small" value={gender}
              ...
              <Grid item xs={6} sm={3} md={2}>
                <TextField fullWidth label="Weight (kg)" size="small" type="number" value={weight}
              ...
              <Grid item xs={6} sm={3} md={2}>
                <TextField fullWidth label="Height (cm)" size="small" type="number" value={height}
              ...
              <Grid item xs={6} sm={3} md={2}>
                <TextField fullWidth label="Age" size="small" type="number" value={age}
              ...
              <Grid item xs={6} sm={3} md={4}>
                <TextField
                  select fullWidth label="Activity Level" size="small" value={activity}
              ...
              <Box sx={{ mt: 3 }}>
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} md={4}>
                    ...
                    <Box sx={{ display: "flex", gap: 2, mt: 1.5, flexWrap: "wrap" }}>
                      {[
                        ["Maintenance", macroCalc.tdee, "#777"],
                        ["Your Target", macroCalc.targetCal, brandColor],
                      ].map(([l, v, c]) => (
                ...
                <Box sx={{ mt: 2, p: 1.5, bgcolor: "#0d0d0d", borderRadius: 2, borderLeft: `3px solid ${selectedGoal.color}` }}>
                  <Typography variant="caption" sx={{ color: "#888", lineHeight: 1.6 }}>
                    ...
                    BMR (base metabolic rate): {macroCalc.bmr} kcal · TDEE (maintenance): {macroCalc.tdee} kcal
                  </Typography>
                </Box>
              </Box>
            ) : (
                ...
                <Card sx={{ bgcolor: "#111", border: "1px solid #222", borderRadius: 2, height: "100%" }}>
                  <CardContent sx={{ p: 2.5 }}>
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, mb: 2 }}>
                      {bt.traits.map(t => (
                        <Chip key={t} label={t} size="small" sx={{ bgcolor: "#1e1e1e", color: "#aaa", fontSize: "0.65rem", height: 20 }} />
                      ))}
                    </Box>
        ...
        <Box sx={{ mb: 5 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 0.5, flexWrap: "wrap" }}>
            <Typography variant="h5" fontWeight={900}>
              {selectedGoal.emoji} {selectedGoal.label} — Food Guide
            </Typography>
            <Chip label={`${foods.length} options`} size="small" sx={{ bgcolor: selectedGoal.color, color: "#fff", fontWeight: 700 }} />
          </Box>
          <Typography variant="body2" sx={{ color: "#777", mb: 3 }}>
            25 realistic, easy-to-make options per meal category.
          </Typography>

          <Tabs
            value={mealTab}
            onChange={(_, v) => setMealTab(v)}
            variant="scrollable"
            scrollButtons="auto"
            sx={{
              mb: 3,
          ...
          >
            {MEAL_TYPES.map(mt => (
              <Tab key={mt.id} label={`${mt.emoji} ${mt.label}`} />
            ))}
          </Tabs>
          ...
          <Grid container spacing={1.5}>
            {foods.map((food, i) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={i}>
                <FoodCard food={food} color={selectedGoal.color} />
              </Grid>
            ))}
          </Grid>
          ...
          <Box sx={{ mt: 2, p: 1.5, bgcolor: "#0d0d0d", borderRadius: 2, border: "1px solid #1e1e1e" }}>
            <Typography variant="caption" sx={{ color: "#555" }}>
              🔵 Protein &nbsp;·&nbsp; 🟡 Carbs &nbsp;·&nbsp; 🔴 Fat &nbsp;·&nbsp; Macros are per typical serving. Adjust portions to hit your daily calorie and macro targets above.
            </Typography>
          </Box>
        </Box>
        ...
        <Box sx={{ mb: 5 }}>
          ...
          <Typography variant="body2" sx={{ color: "#777", mb: 3 }}>
            Evidence-based advice on timing, hydration, late-night eating, and more.
          </Typography>
          ...
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 3 }}>
            {guideSections.map((s, i) => (
              <Chip
                ...
                sx={{
                  bgcolor: guideSection === i ? brandColor : "#1a1a1a",
                  color: guideSection === i ? "#fff" : "#888",
                  fontWeight: 700,
                  cursor: "pointer",
                  border: "1px solid",
                  borderColor: guideSection === i ? brandColor : "#2a2a2a",
                  transition: "all .2s",
                }}
              />
            ))}
        ...
        <Box sx={{ mb: 5 }}>
          ...
          <Typography variant="body2" sx={{ color: "#777", mb: 3 }}>
            The pros, cons, and honest verdicts on every drink you might reach for.
          </Typography>
          <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6} key={i}>
                <Card sx={{ bgcolor: "#111", border: `1px solid ${drink.color}22`, borderRadius: 2, height: "100%" }}>
                  <CardContent sx={{ p: 2.5 }}>
                    ...
                    <Box sx={{ mb: 1 }}>
                      {drink.pros.map((p, j) => (
                        <Typography key={j} variant="caption" sx={{ color: "#4caf80", display: "block", lineHeight: 1.6 }}>✓ {p}</Typography>
                      ))}
                    </Box>
                    <Box sx={{ mb: 1.5 }}>
                      {drink.cons.map((c, j) => (
                        <Typography key={j} variant="caption" sx={{ color: "#e05c5c", display: "block", lineHeight: 1.6 }}>✗ {c}</Typography>
                      ))}
                    </Box>
                    <Box sx={{ p: 1.5, bgcolor: "#0d0d0d", borderRadius: 1.5, borderLeft: `3px solid ${drink.color}` }}>
                      <Typography variant="caption" sx={{ color: "#aaa", lineHeight: 1.6 }}>
                        <b style={{ color: drink.color }}>Verdict: </b>{drink.verdict}
                      </Typography>
                    </Box>
                  </CardContent>
                </Card>
        ...
        <Box sx={{ mb: 5 }}>
          ...
          <Typography variant="body2" sx={{ color: "#777", mb: 3 }}>
            BMI is a useful screening tool, not a definitive health measure. Use it alongside body composition, fitness level, and how you feel.
          </Typography>
          <Grid container spacing={2}>
        ...
        {/* Footer */}
        <Box sx={{ pt: 3, borderTop: "1px solid #1e1e1e", textAlign: "center" }}>
          <Typography variant="caption" sx={{ color: "#333" }}>
            ...
            {trainerInfo?.name && ` · Provided by ${trainerInfo.name}`}
          </Typography>
        </Box>
