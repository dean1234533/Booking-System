  ...
  if (view === "list") return (
    <Box>
      ...
      {loading ? (
        ...
        <Stack spacing={1.5}>
          {palettes.map(pal => (
            <Box key={pal.id} sx={{ bgcolor: "#fff", border: "1px solid #e5e7eb", p: 2.5, display: "flex", alignItems: "center", gap: 2, borderRadius: "8px" }}>
              {/* Swatches */}
              <Box sx={{ display: "flex", gap: 0.5, flexShrink: 0 }}>
                {(pal.colours || []).slice(0, 7).map((c, i) => (
                  <Tooltip key={i} title={`${c.name ? c.name + " · " : ""}${c.hex?.toUpperCase()}`}>
                    <Box sx={{ width: 26, height: 26, bgcolor: c.hex, border: "1px solid #e5e7eb", borderRadius: "4px" }} />
                  </Tooltip>
                ))}
              </Box>
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography sx={{ fontFamily: SANS, fontWeight: 700, color: "#111", fontSize: "0.88rem" }}>{pal.name}</Typography>
  ...
  if (view === "create") return (
    <Box>
      ...
      <Box sx={{ maxWidth: 620 }}>
        ...
        <Typography sx={{ fontFamily: SANS, fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(0,0,0,0.4)", mb: 1.5 }}>
          Colours
        </Typography>

        <Stack spacing={1.5} sx={{ mb: 2.5 }}>
          {colours.map(c => (
            <Box key={c.id} sx={{ display: "flex", alignItems: "center", gap: 2, bgcolor: "#f8f9fa", border: "1px solid #e5e7eb", p: 2, borderRadius: "8px" }}>
              ...
              <Typography sx={{ fontFamily: "'Courier New', monospace", fontSize: "0.75rem", color: "rgba(0,0,0,0.45)", width: 72, flexShrink: 0, letterSpacing: "0.04em" }}>
                {(c.hex || "").toUpperCase()}
              </Typography>

              <IconButton size="small" onClick={() => setColours(p => p.filter(x => x.id !== c.id))}
                sx={{ color: "rgba(0,0,0,0.3)", flexShrink: 0, "&:hover": { color: "#ff6b6b" } }}>
                <DeleteIcon sx={{ fontSize: 16 }} />
              </IconButton>
            </Box>
          ))}
        ...
        <Stack direction="row" spacing={1.5}>
          <Button onClick={() => setView("list")}
            sx={{ color: "rgba(0,0,0,0.5)", fontWeight: 600, fontSize: "0.76rem" }}>
            Cancel
          </Button>
  ...
  return (
    <Box>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 3 }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography sx={{ fontFamily: SERIF, fontSize: "1.4rem", color: "#111", fontWeight: 400 }}>{selected?.name}</Typography>
          <Typography sx={{ fontFamily: SANS, fontSize: "0.7rem", color: "rgba(0,0,0,0.45)" }}>Client responses</Typography>
        </Box>
        ...
        {(selected?.colours || []).map((c, i) => (
          <Tooltip key={i} title={`${c.name || ""} ${c.hex?.toUpperCase()}`}>
            <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 0.5 }}>
              <Box sx={{ width: 50, height: 50, bgcolor: c.hex, border: "1px solid #e5e7eb", borderRadius: "6px" }} />
              <Typography sx={{ fontFamily: SANS, fontSize: "0.58rem", color: "rgba(0,0,0,0.45)", maxWidth: 50, textAlign: "center", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      ...
      {appLoading ? (
        ...
        <Box sx={{ textAlign: "center", py: 7, color: "rgba(0,0,0,0.3)" }}>
        ...
        <Stack spacing={1.5}>
          {approvals.map(a => (
            <Box key={a.id} sx={{ bgcolor: "#fff", border: "1px solid #e5e7eb", p: 2.5, borderRadius: "8px" }}>
              <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                  <CheckCircleIcon sx={{ fontSize: 17, color: "#16a34a" }} />
                  <Typography sx={{ fontFamily: SANS, fontWeight: 700, color: "#111", fontSize: "0.88rem" }}>{a.clientName}</Typography>
                  <Chip label="Approved" size="small" sx={{ bgcolor: "#dcfce7", color: "#16a34a", fontSize: "0.62rem", height: 19, borderRadius: 0 }} />
                </Box>
              ...
              {a.feedback && (
                <Typography sx={{ fontFamily: SANS, fontSize: "0.82rem", color: "rgba(0,0,0,0.55)", fontStyle: "italic", mt: 1, pl: 3.5, lineHeight: 1.55 }}>
                  "{a.feedback}"
                </Typography>
              )}
              <Typography sx={{ fontFamily: SANS, fontSize: "0.64rem", color: "rgba(0,0,0,0.35)", mt: 1, pl: 3.5 }}>
